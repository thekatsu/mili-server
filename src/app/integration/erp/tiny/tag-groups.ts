...
export async function getTagGroups() {
  ...
  let estimatedRecords = await db.tagGroup.count();
  ...
  for (let pagina = 1; pagina <= paginas; pagina++) {
    ...
    const httpResponse = await fetch(
      `http://api.tiny.com.br/api2/grupo.tag.pesquisa.php?token=${TOKEN}&pesquisa=${pesquisa}&situacao=A&pagina=${pagina}&formato=json`,
      {
        method: 'POST',
      },
    );
